...
ROOT=Path(__file__).resolve().parents[1];work=ROOT/'output/music-second';work.mkdir(parents=True,exist_ok=True)
source=work/'cipher-source.mp3'
...
output=ROOT/'public/audio/cipher-car-dj.mp3'
subprocess.run(['ffmpeg','-v','error','-y','-i',str(source),'-af',eq+','+norm,'-ar','44100','-ac','2','-c:a','libmp3lame','-b:a','192k','-map_metadata','-1','-metadata','title=Cipher - Car DJ bass edit','-metadata','artist=Kevin MacLeod','-metadata','comment=CC BY 4.0; low-frequency EQ and loudness adaptation for Velocity Zero',str(output)],check=True)
print(json.dumps({'file':str(output),'bytes':output.stat().st_size,'sha256':hashlib.sha256(output.read_bytes()).hexdigest(),'sourceSha256':hashlib.sha256(source.read_bytes()).hexdigest(),'bpm':150},indent=2))
