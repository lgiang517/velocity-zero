...
const keys=new Set(),audio=new DriveAudio();let accumulator=0,lastFrame=0,frames=0,frameTime=0,fps=60;
...
function saveSettings(){try{localStorage.setItem('velocity-zero-settings',JSON.stringify({car:config.id,paint,weather:$('weather').value,assists:$('assists').value,quality:$('quality').value,volume:$('volume').value,music:$('music').value}));}catch{}}
...
function rebuildCar(){
  ...
  player=new VehiclePhysics(config);player.reset(startS,-2.6);car=createCar(config,paint);world.scene.add(car.root);
...
async function startRace(){
  ...
  clearOpponents();cameraMode=0;startS=raceMode.start?track.length*raceMode.start:45;finishS=startS+track.length*(raceMode.length||raceMode.laps)-40;
  ...
  $('hud-mode').textContent=raceMode.name==='SPRINT'?'COASTAL SPRINT':raceMode.name;$('camera-label').textContent='CHASE CAM';$('countdown').textContent='';world.initializedCamera=false;
...
function pause(){if(state==='race'||state==='countdown'){pausedState=state;state='paused';keys.clear();show('pause-screen',true);}else if(state==='paused'){state=pausedState;show('pause-screen',false);lastFrame=performance.now();}}
function toMenu(){state='menu';keys.clear();clearOpponents();show('hud',false);show('pause-screen',false);show('results',false);show('menu',true);document.body.classList.remove('racing');startS=45;player.reset(startS,-2.6);world.weather=$('weather').value;world.initializedCamera=false;selectPanel('drive');}
function finish(caught=false){
  if(state!=='race')return;state='finished';keys.clear();show('results',true);show('hud',false);
...
function step(dt){
  ...
  const q=track.sample(player.s);player.step(dt,readInput(),{curvature:q.curvature,slope:q.slope,wet:world.wet},assist);
...
function bindUI(){
  ...
  window.addEventListener('keydown',e=>{
    ...
    if(e.code==='KeyC'&&(state==='race'||state==='countdown')){cameraMode=(cameraMode+1)%4;world.initializedCamera=false;$('camera-label').textContent=['CHASE CAM','HOOD CAM','COCKPIT CAM','CINEMA CAM'][cameraMode];}
    if(e.code==='KeyR'&&state==='race'){player.d=-2.6;player.yaw=0;player.v=0;player.r=0;player.u=Math.min(player.u,12);raceTime+=3;notice('BACK ON THE ROAD  +3 SEC');world.initializedCamera=false;}
    keys.add(e.code);
  });
  window.addEventListener('keyup',e=>keys.delete(e.code));window.addEventListener('blur',()=>{keys.clear();if(state==='race'||state==='countdown')pause();});document.addEventListener('visibilitychange',()=>{if(document.hidden&&(state==='race'||state==='countdown'))pause();});window.addEventListener('resize',()=>world.resize());
...
async function init(){
  try{
    track=new CoastTrack();world=new GameWorld($('world'),track);config=CARS.find(c=>c.id===storage.car)||CARS[0];paint=storage.paint||paint;rebuildCar();refreshGarage();refreshModes();bindUI();
    for(const id of['weather','assists','quality','volume','music'])if(storage[id]!==undefined)$(id).value=storage[id];
    world.weather=$('weather').value;$('weather').onchange();assist=$('assists').value==='on';world.setQuality($('quality').value);audio.volume=$('volume').value/100;audio.musicVolume=$('music').value/100;
