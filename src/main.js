...
function setCamera(mode){cameraMode=Number.isInteger(Number(mode))?clamp(Number(mode),0,3):0;$('camera').value=String(cameraMode);$('camera-label').textContent=t(cameraLabels[cameraMode]);world.initializedCamera=false;saveSettings();}
...
async function startRace(){
  ...
  await audio.init();audio.setEnabled(true);$('audio-toggle').setAttribute('aria-label',t('Mute audio'));document.querySelector('.audio-slash').style.display='none';
}
...
function bindUI(){
  $('language').value=getLanguage();$('language').onchange=()=>{changeLanguage($('language').value);refreshGarage();refreshModes();$('world-weather').textContent=t(({sunset:'GOLDEN HOUR',clear:'CLEAR SKIES',night:'MIDNIGHT',rain:'LIGHT RAIN',storm:'STORM FRONT',fog:'COASTAL FOG',dynamic:'CHANGING SKIES'})[world.weather]);$('camera-label').textContent=t(['CHASE CAM','HOOD CAM','DRIVER CAM','CINEMA CAM'][cameraMode]);};
  ...
  window.addEventListener('keydown',e=>{
    ...
    if(e.code==='KeyR'&&state==='race'){drivingInput.reset();player.steer=0;player.d=-2.6;player.yaw=0;player.v=0;player.r=0;player.u=Math.min(player.u,12);raceTime+=3;notice(t('BACK ON THE ROAD  +3 SEC'));world.initializedCamera=false;}
    keys.add(e.code);
  });
...
async function init(){
  try{
    initLanguage();track=new CoastTrack();world=new GameWorld($('world'),track);config=CARS.find(c=>c.id===storage.car)||CARS[0];paint=storage.paint||paint;await loadCarModels();rebuildCar();refreshGarage();refreshModes();bindUI();
