...
export class CoastTrack {
  ...
  roadRibbon(from=-8.4,to=8.4,height=0){
    const columns=[from,...[-7.65,-3.85,-.12,0,.12,3.85,7.65].filter(d=>d>from&&d<to),to];
    const stations=[0];
    ...
    const segments=stations.length-1,stride=columns.length,pos=[],uv=[],indices=[];
    for(let i=0;i<=segments;i++){
      const t=stations[i]/this.length,p=this.curve.getPointAt(i===segments?0:t),tan=this.curve.getTangentAt(i===segments?0:t);
      const right=new THREE.Vector3(tan.z,0,-tan.x).normalize();
      for(const d of columns){pos.push(p.x+right.x*d,p.y+height,p.z+right.z*d);uv.push(d,t*this.length);}
    }
