...
function fixture(t){
 ...
 const param=()=>({value:0,target:null,setTargetAtTime(value,time,constant){this.target=value;this.schedule={time,constant};}}),gain=()=>({gain:param(),connections:[],connect(node){this.connections.push(node);},disconnect(){this.disconnected=true;}});
 const a=new DriveAudio();a.ready=true;a.enabled=true;a.master=gain();a.musicBus=gain();a.ctx={state:'running',currentTime:0,sampleRate:44100,destination:{},resume(){return Promise.resolve();},close(){return Promise.resolve();},createGain:gain,createMediaElementSource(element){const n={element,connections:[],connect(node){this.connections.push(node);},disconnect(){this.disconnected=true;}};nodes.push(n);return n;},createBuffer(){return{};},createBufferSource(){return{connect(){},start(){},disconnect(){}};},decodeAudioData(){throw Error('Music must never allocate decoded PCM');}};
 return {a,nodes};
}
test('music streams through one shared media node and an isolated music boost with no PCM decode',async t=>{
 const {a,nodes}=fixture(t);await a.loadMusic();const e=a.musicElement;await a.loadMusic();assert.equal(nodes.length,1);assert.equal(e.loads,1);assert.equal(a.musicGain.gain.value,2);assert.deepEqual(nodes[0].connections,[a.musicGain]);assert.deepEqual(a.musicGain.connections,[a.musicBus]);assert.ok(e.src.endsWith('xiang-zhe-hai.mp3'));a.updateMusic(true);await tick();assert.equal(e.paused,false);assert.ok(a.musicSource);await a.dispose();assert.equal(nodes[0].disconnected,true);assert.equal(e.src,'');
});
