...
test('landform skirts match the base and cannot bury adjacent switchback shelves',()=>{
 ...
 for(let s=section.startS;s<section.endS;s+=19)for(const lateral of[-30,-10,0,10,30]){const p=world.track.point(s,lateral),base=world.groundHeight(p.x,p.z);assert.ok(scene.landforms.heightAt(p.x,p.z,base)<=base+.001);}
...
test('all Duku-owned geometry is disposed without removing existing terrain',()=>{
 const existing=world.scene.children.find(m=>!m.userData.dukuScenery);let releases=0;for(const mesh of scene.meshes)mesh.geometry.addEventListener('dispose',()=>releases++);scene.dispose();assert.equal(releases,scene.meshes.length);assert.ok(world.scene.children.includes(existing));assert.ok(scene.meshes.every(m=>!world.scene.children.includes(m)));
});
